...
description: Builds parserless Figma Code Connect templates (.figma.js) that map published library components to code snippets using Figma MCP tools get_code_connect_suggestions and get_context_for_code_connect. Use when the user mentions Code Connect, Figma component mapping, design-to-code, Dev Mode snippets, or creating or updating .figma.js files. Does not cover parser-based figma.connect() .figma.tsx workflows published via CLI.
---

## Overview

Guide the user through parserless Code Connect: `.figma.js` templates that map Figma components to code snippets, using Figma MCP to discover unmapped components and fetch property definitions. Given a Figma URL with `node-id`, follow the steps below.

## Install in Cursor

Copy this directory to either:

- **All projects:** `~/.cursor/skills/figma-code-connect/` (create `skills` if needed)
- **One repo:** `<repository-root>/.cursor/skills/figma-code-connect/`

Restart Cursor or reload skills so the agent can discover the skill.
...
Use the tagged template that matches the target stack:
...
| Need | Open |
|------|------|
| Full `instance.*` signatures, `InstanceHandle`, `TextHandle`, `SelectorOptions`, export shape | [references/api.md](references/api.md) |
| Deep nesting, metadata props, `findConnectedInstances` | [references/advanced-patterns.md](references/advanced-patterns.md) |

**Reminders:** `findInstance` / `findConnectedInstance` / `findText` can return a truthy `ErrorHandle` — use `type === 'INSTANCE'` before `hasCodeConnect()`. `executeTemplate().example` is a `ResultSection[]`, not a string; never `+` or `.join()` it outside a tagged template.
